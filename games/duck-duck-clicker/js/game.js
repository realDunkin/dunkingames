...
function onDucketChanged() {
    // update the label
    ducketsLabel.textContent = duckets.toFormat(0) + " DUCKET$"
...
function onAscensionChanged() {
    if (ascension.isGreaterThanOrEqualTo("1")){
        ...
        // update the label
        ascensionLabel.textContent = "ASCEND: X" + ascension.toFormat(2)
...
function init() {
    ...
    if (localStorage.hasOwnProperty("t")) {
        ...
        showNotification("img/UI/wave.svg", null, `WELCOME BACK! YOU HAVE MADE ${amount.toFormat(0)} DUCKET$ WHILE YOU WERE AWAY!`)
