...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    constructor(c) {
      ...
      this.AddRuntimeMessageHandler("vibrate", (a) =>
        this._OnGamepadVibrate(a)
      );
      this.AddRuntimeMessageHandler("reset-vibrate", (a) =>
        this._OnGamepadResetVibrate(a)
      );
      window.addEventListener("gamepadconnected", (a) =>
        this._OnGamepadConnected(a)
      );
      window.addEventListener("gamepaddisconnected", (a) =>
        this._OnGamepadDisconnected(a)
      );
    ...
    _OnReady() {
      ...
      for (const c of this._GetActiveGamepads())
        this.PostToRuntime("gamepad-connected", { index: c.index, id: c.id });
    ...
    _OnGamepadDisconnected(c) {
      this._isReady &&
        this.PostToRuntime("gamepad-disconnected", { index: c.gamepad.index });
    }
    Tick() {
    ...
    _GetGamepadByIndex(c) {
      for (const a of this._GetActiveGamepads()) if (a.index === c) return a;
      return null;
    }
    async _OnGamepadVibrate(c) {
      var a = this._GetGamepadByIndex(c.index);
    ...
    _OnGamepadResetVibrate(c) {
      (c = this._GetGamepadByIndex(c.index)) &&
        (c = c.vibrationActuator) &&
        c.reset &&
        c.reset();
    }
    _OnWindowUnload() {
